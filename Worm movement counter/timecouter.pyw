...
# Record for the current worm
last_record = [   # (direction, time_interver)
    ]

# All record
list_record = [   # (No,(total_counts, total_accumulations, forward_counts, forward_accumulations, backward_counts, backward_accumulations, others_counts, others_accumulations),
...
forward_counts = IntVar()
forward_accumulations = IntVar()
backward_counts = IntVar()
backward_accumulations = IntVar()
...
# Icons
dict_image = {
    'Up':     ['Up',      'forward.png',   None],
    'Down':   ['Down',    'backward.png',  None],
    'Left':   ['Left',    'others.png',    None],
    'Pause':  ['Pause',   'pause.png',     None],
...
def key_Pause(event_keysym, time, key):
    ...
    elif event_keysym != 'Pause': # non Pause key is pressed when pausing
...
def key_Movement(event_keysym, time, key):
    ...
    if len(last_movement) != 0 and key[0] != last_movement[0]:
        timecounter_thisRec.setvalue('00:00:00')
...
def key_Next(event_keysym, time, key):
    ...
    if len(last_movement) != 0:
        ...
        rpt = u'\nNo movement for the current number of worm is recorded; Number will not be added'
        print_report(text_intime, rpt)
        return

    # Stop recording for the current number of worm, reset data and interface
    list_record.append((\
        recordNo.get(), (total_counts.get(), total_accumulations.get(), \
        forward_counts.get(), forward_accumulations.get(), backward_counts.get(), \
        backward_accumulations.get(), others_counts.get(), others_accumulations.get()), \
        last_record[:]
        ))
    ...
    forward_counts.set(0)
    forward_accumulations.set(0)
    backward_counts.set(0)
    backward_accumulations.set(0)
    others_counts.set(0)
    ...
    # Print the recording of the current number of worm to screen
    rpt = u'\nRecord No.%s\n' % list_record[-1][0]
    rpt += '='*24 + '\n'
    rpt += u'Total times:%10s\nTotal duration:%6s\nForward times:%10s\nForward duration:%6s\
    \nBackward times:%10s\nBackward duration:%6s\nRest times:%10s\nRest duration:%6s\n' % \
    list_record[-1][1]
    rpt += '='*24 + '\n'
...
dict_key   = {
    'Up':     ['Up',      u'Forward',                 key_Movement, forward_counts,  forward_accumulations],
    'Down':   ['Down',    u'Backward',                 key_Movement, backward_counts, backward_accumulations],
    'Left':   ['Left',    u'Rest',                 key_Movement, others_counts,   others_accumulations],
    'Pause':  ['Pause',   u'Pause',                 key_Pause],
...
def selectFileName():
    ofile = tkFileDialog.asksaveasfilename(filetypes=[(u'add.csv extension automatically','*')])
...
def doSave_(ofile):
    ...
    rpt = u'No., Total times, Total duration, Forward times, Forward duration, Backward times, Backward duration, Rest times, Rest duration\r\n'
...
def save2File():
    ...
    if ofile == '': ofile = tkFileDialog.asksaveasfilename(filetypes=[(u'add extension .csv','*')])
    if ofile == '': return
    else:
...
text_recordNo = Pmw.Counter(frame_top, labelpos=W, label_text=u'Worm Number',\
...
label_forward = Label(frame_statistics_top, text=u'Forward:')
text_forward_counts = Entry(frame_statistics_top, textvariable=forward_counts, width=8)#Var bound
text_forward_accumulations = Entry(frame_statistics_top, textvariable=forward_accumulations, width=8)#Var bound

label_backward = Label(frame_statistics_top, text=u'Backward:')
text_backward_counts = Entry(frame_statistics_top, textvariable=backward_counts, width=8)#Var bound
text_backward_accumulations = Entry(frame_statistics_top, textvariable=backward_accumulations, width=8)#Var bound
...
timecounter_total = Pmw.TimeCounter(frame_statistics_top, label_text=u'Current Worm No.', labelpos=NW, \
...
label_forward.grid(row=1, column=0, sticky=EW)
text_forward_counts.grid(row=1, column=1, sticky=EW)
text_forward_accumulations.grid(row=1, column=2, sticky=EW)

label_backward.grid(row=2, column=0, sticky=EW)
text_backward_counts.grid(row=2, column=1, sticky=EW)
text_backward_accumulations.grid(row=2, column=2, sticky=EW)
...
print_report(text_output, u'Movement recording for current worm:\n')
